	...
	public class InventoryWindow : MonoBehaviour
	{
		...
		public void AddPop_up(vWindowPop_up pop_up)
		{
			if (!pop_ups.Contains(pop_up))
			{
				...
				{
					pop_up.gameObject.SetActive(value: true);
				}
			}
		}
		...
		public void RemovePop_up(vWindowPop_up pop_up)
		{
			try
			{
				if (pop_ups.Contains(pop_up))
				{
					...
					if (pop_up.gameObject.activeSelf)
					{
						pop_up.gameObject.SetActive(value: false);
					}
				}
			}
			catch
		...
		public void RemoveLastPop_up()
		{
			if (pop_ups.Count > 0)
			{
				vWindowPop_up vWindowPop_up = pop_ups[pop_ups.Count - 1];
				pop_ups.Remove(vWindowPop_up);
				if (vWindowPop_up.gameObject.activeSelf)
				{
					vWindowPop_up.gameObject.SetActive(value: false);
				}
				if (pop_ups.Count > 0 && pop_ups.Count > 0 && !pop_ups[pop_ups.Count - 1].gameObject.activeSelf)
				{
					pop_ups[pop_ups.Count - 1].gameObject.SetActive(value: true);
				}
			}
		}
