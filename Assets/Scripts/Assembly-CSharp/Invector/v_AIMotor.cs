...
namespace Invector
{
	public class v_AIMotor : vCharacter
	{
		...
		public bool actions => lockMovement || isRolling;
		...
		public bool OnCombatArea
		{
			get
			{
				...
				Vector3 a = new Vector3(0f, position.y, 0f);
				Vector3 position2 = target.position;
				return Vector3.Distance(a, new Vector3(0f, position2.y, 0f)) < distanceToAttack && agressiveAtFirstSight && TargetDistance <= strafeDistance && !agent.isOnOffMeshLink;
			}
		}
		...
		public bool OnStrafeArea
		{
			get
			{
				...
				Vector3 a = new Vector3(0f, position.y, 0f);
				Vector3 position2 = target.position;
				bool flag = Vector3.Distance(a, new Vector3(0f, position2.y, 0f)) < 1.5f;
				if (isStrafing)
				{
		...
		protected void CheckGroundDistance()
		{
			...
			float num = 10f;
			Ray ray = new Ray(base.transform.position + new Vector3(0f, _capsuleCollider.height / 2f, 0f), Vector3.down);
			if (Physics.Raycast(ray, out groundHit, _capsuleCollider.height * 0.75f, groundLayer))
			{
				Vector3 position = base.transform.position;
				float y = position.y;
			...
			{
				if (currentHealth > 0f && NavMesh.SamplePosition(base.transform.position, out NavMeshHit _, 5f, -1))
				{
		...
		public void CheckAutoCrouch()
		{
			...
			if (Physics.SphereCast(ray, radius, out RaycastHit _, headDetect - _capsuleCollider.radius * 0.1f, autoCrouchLayer))
			{
				isCrouched = true;
			}
			else
		...
		public bool onFovAngle()
		{
			...
			Vector3 eulerAngle = Quaternion.LookRotation(target.position - base.transform.position, Vector3.up).eulerAngles - base.transform.eulerAngles;
			Vector3 vector = eulerAngle.NormalizeAngle();
			fovAngle = vector.y;
		...
		public void CheckTarget()
		{
			...
			Vector3 center = colliderTarget.bounds.center;
			float x = center.x;
			Vector3 max = colliderTarget.bounds.max;
			float y = max.y;
			Vector3 center2 = colliderTarget.bounds.center;
			Vector3 vector = new Vector3(x, y, center2.z);
			Vector3 center3 = colliderTarget.bounds.center;
			float x2 = center3.x;
			Vector3 min = colliderTarget.bounds.min;
			float y2 = min.y;
			Vector3 center4 = colliderTarget.bounds.center;
			Vector3 vector2 = new Vector3(x2, y2, center4.z);
			...
			if (canSeeTargetIteration == 0 && !Physics.Linecast(head.position, vector, out RaycastHit hitInfo, sphereSensor.obstacleLayer))
			{
				canSeeTarget = true;
				canSeeTargetIteration = 0;
				return;
			}
			if (canSeeTargetIteration == 1 && !Physics.Linecast(head.position, vector2, out hitInfo, sphereSensor.obstacleLayer))
			{
				canSeeTarget = true;
				canSeeTargetIteration = 0;
				return;
			}
			if (canSeeTargetIteration == 2 && !Physics.Linecast(head.position, colliderTarget.bounds.center, out hitInfo, sphereSensor.obstacleLayer))
			{
				canSeeTarget = true;
				canSeeTargetIteration = 0;
				return;
		...
		{
			if (isRolling || currentHealth <= 0f || !animator.enabled || (!damage.ignoreDefense && !actions && CheckChanceToRoll()))
			{
				return;
			}
			Vector3 position = base.transform.position;
		...
		protected bool CheckChanceToRoll()
		{
			...
			if (num < chanceToRoll && num > 0f && target != null)
			{
				animator.SetTrigger("ResetState");
				sideMovement = GetRandonSide();
				rollDirection = new Ray(target.position, target.right * sideMovement).direction;
		...
		protected IEnumerator CheckChanceToBlock(float chance, float timeToEnter)
		{
			tryingBlock = true;
			System.Random random = new System.Random();
			float randomBlock = (float)random.NextDouble();
			if (randomBlock < chance && randomBlock > 0f && !isBlocking)
			{
				if (timeToEnter > 0f)
				{
					yield return new WaitForSeconds(timeToEnter);
				}
				isBlocking = ((!(target == null) && !actions && !isAttacking) ? true : false);
		...
		{
			yield return new WaitForSeconds((!(target == null)) ? raiseShield : 0f);
			isBlocking = false;
		}
