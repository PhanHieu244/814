...
namespace Invector
{
	public abstract class vLockOnTarget : MonoBehaviour
	{
		...
		public string[] tagsToFind = new string[1]
		{
			"Enemy"
		};
		...
		public virtual Transform currentTarget => target;
		...
		private IEnumerator ChangeTargetRoutine(int value)
		{
			if (changingTarget)
			{
				yield break;
		...
		public virtual bool isCharacterAlive()
		{
			...
			vCharacter component = currentTarget.GetComponent<vCharacter>();
			if (component == null)
			{
				return false;
			}
			if (component.ragdolled)
			{
				return false;
			}
		...
		{
			vCharacter component = other.GetComponent<vCharacter>();
			if (component == null)
			{
				return false;
			}
		...
		protected void OnDrawGizmos()
		{
			if (showDebug && (bool)watcher)
			{
				...
				if (visibles != null && visibles.Count > 0 && target != null)
				{
					...
					{
						Gizmos.color = ((!_transform.Equals(currentTarget)) ? Color.yellow : Color.red);
		...
		protected List<Transform> GetPossibleTargets()
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				{
					if (Physics.Linecast(base.transform.position, end, out RaycastHit hitInfo, layerOfObstacles))
					{
		...
		protected void SortTargets(ref List<Transform> list)
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				Vector2 point = cam.WorldToScreenPoint(transform.transform.position);
			...
			{
				Vector2 a3 = cam.WorldToScreenPoint(t1.transform.position);
				Vector2 a4 = cam.WorldToScreenPoint(t2.transform.position);
			...
			{
				Vector2 a = cam.WorldToScreenPoint(t1.transform.position);
				Vector2 a2 = cam.WorldToScreenPoint(t2.transform.position);
